	...
	if ($act == 'print')
	{
		...
		$content = "<table style='padding-bottom: 10px; width: 240mm;'>
					...
					<table cellpadding='0' cellspacing='0' style='width: 290mm;'>
						...
						if ($sDate != '' || $eDate != '')
						{
							$queryRetur = "SELECT * FROM as_retur_suppliers WHERE returNo LIKE '%$q%' AND returDate BETWEEN '$startDate' AND '$endDate' ORDER BY returDate ASC";
						}
						else
						{
							$queryRetur = "SELECT * FROM as_retur_suppliers WHERE returNo LIKE '%$q%' ORDER BY returDate ASC";
						}
						...
						while ($dtRetur = mysqli_fetch_array($sqlRetur))
						{
							...
							$queryDetail = "SELECT * FROM as_detail_retur_suppliers WHERE returNo = '$dtRetur[returNo]' AND returID = '$dtRetur[returID]'";
