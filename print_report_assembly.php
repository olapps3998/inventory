	...
	if ($act == 'print')
	{
		...
		$content = "<table style='padding-bottom: 10px; width: 240mm;'>
						...
						<tr valign='top'>
							...
							<td>
								<span style='font-size: 11pt;'><b>LAPORAN ASSEMBLY</b><br>Periode : $_GET[startDate] s/d $_GET[endDate]</span>
							</td>
						</tr>
					</table>
					<table cellpadding='0' cellspacing='0' style='width: 290mm;'>
						...
						{
							$queryAssembly = "SELECT * FROM as_assembly WHERE assemblyDate BETWEEN '$startDate' AND '$endDate' ORDER BY assemblyDate DESC";
						}
						else
						{
							$queryAssembly = "SELECT * FROM as_assembly ORDER BY assemblyDate DESC";
						}
