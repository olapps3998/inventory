...
if ($_SESSION['staffID'] == '' && $_SESSION['email'] == '')
{
	...
	if ($act == 'print')
	{
		...
		$filename="pembayaran_transaksi_pembelian.pdf";
		...
		$content = "<table style='padding-bottom: 10px; width: 240mm;'>
					...
					<table cellpadding='0' cellspacing='0' style='width: 290mm;'>
						<tr>
							...
							<th style='width: 25mm; padding: 5px 0px 5px 0px; font-size: 9pt; border-top: 1px solid #000; border-bottom: 1px solid #000;'>NO PO</th>
						...
						while ($dtPay = mysqli_fetch_array($sqlPay))
						{
							...
							$queryBuy = "SELECT valas FROM as_buy_transactions WHERE invoiceNo = '$dtPay[invoiceNo]' AND spbNo = '$dtPay[spbNo]'";
							$sqlBuy = mysqli_query($connect, $queryBuy);
							$dataBuy = mysqli_fetch_array($sqlBuy);
							
							$paymentDate = tgl_indo2($dtPay['paymentDate']);
							$total = $dtPay['total']; //$total = rupiah($dtPay['total']);
							...
							$content .= "
									<tr valign='top'>
										...
										<td style='padding: 2px 0px 2px 0px; font-size: 9pt;'>$dtPay[spbNo]</td>
										<td style='padding: 2px 0px 2px 0px; font-size: 9pt;'>$payType</td>
										<td style='padding: 2px 0px 2px 0px; font-size: 9pt;'>$dataBuy[valas]</td>
