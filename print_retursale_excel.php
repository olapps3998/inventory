	...
	if ($act == 'print')
	{
		...
		$content = "<table style='padding-bottom: 10px; width: 240mm;'>
						...
						<tr valign='top'>
							...
							<td>
								<span style='font-size: 11pt;'><b>RETUR PENJUALAN</b></span>
							</td>
						</tr>
					</table>
					<table cellpadding='0' cellspacing='0' style='width: 290mm;'>
						<tr>
							...
							<th style='width: 23mm; padding: 5px 0px 5px 0px; font-size: 9pt; border-top: 1px solid #000; border-bottom: 1px solid #000;'>NO RETUR</th>
							<th style='width: 23mm; padding: 5px 0px 5px 0px; font-size: 9pt; border-top: 1px solid #000; border-bottom: 1px solid #000;'>TGL</th>
							<th style='width: 22mm; padding: 5px 0px 5px 0px; font-size: 9pt; border-top: 1px solid #000; border-bottom: 1px solid #000;'>NO INVOICE</th>
						...
						if ($sDate != '' || $eDate != '')
						{
							$queryRetur = "SELECT * FROM as_retur_staffs WHERE returNo LIKE '%$q%' AND returDate BETWEEN '$startDate' AND '$endDate' ORDER BY returDate ASC";
						}
						else
						{
							$queryRetur = "SELECT * FROM as_retur_staffs WHERE returNo LIKE '%$q%' ORDER BY returDate ASC";
						}
						...
						while ($dtRetur = mysqli_fetch_array($sqlRetur))
						{
							...
							$queryDetail = "SELECT * FROM as_detail_retur_staffs WHERE returNo = '$dtRetur[returNo]' AND returID = '$dtRetur[returID]'";
							...
							$content .= "
									<tr valign='top'>
										...
										<td style='padding: 2px 0px 2px 0px; font-size: 9pt;'>$dtRetur[invoiceNo]</td>
										<td style='padding: 2px 0px 2px 0px; font-size: 9pt;'>$dtRetur[valas]</td>
										<td colspan='4'></td>
									</tr>
							";
							...
							{
								$price = $dtDetail['unitPrice']; //$price = rupiah($dtDetail['unitPrice']);
								$subt = $dtDetail['unitPrice'] * $dtDetail['qty'];
								$sum[] = $subt;
