	...
	if ($act == 'print')
	{
		...
		$content = "<table style='padding-bottom: 10px; width: 240mm;'>
					...
					<table cellpadding='0' cellspacing='0' style='width: 290mm;'>
						...
						if ($sDate != '' || $eDate != '')
						{
							$queryBuy = "SELECT * FROM as_buy_transactions WHERE invoiceNo LIKE '%$q%' AND invoiceDate BETWEEN '$startDate' AND '$endDate' ORDER BY invoiceID DESC";
						}
						else
						{
							$queryBuy = "SELECT * FROM as_buy_transactions WHERE invoiceNo LIKE '%$q%' ORDER BY invoiceID DESC";
						}
						...
						while ($dtBuy = mysqli_fetch_array($sqlBuy))
						{
							...
							$content .= "
									<tr valign='top'>
										...
										<td style='padding: 2px 0px 2px 0px; font-size: 9pt;'>$dtBuy[valas]</td>
										<td colspan='4'></td>
									</tr>
							";
