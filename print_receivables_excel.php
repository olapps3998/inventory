	...
	if ($act == 'print')
	{
		...
		$content = "<table style='padding-bottom: 10px; width: 240mm;'>
					...
					<table cellpadding='0' cellspacing='0' style='width: 240mm; border-bottom: 1px solid #000;'>
						...
						{
							$total = $dataReceive['receiveTotal']; //$total = rupiah($dataReceive['receiveTotal']);
							$incomingTotal = $dataReceive['incomingTotal']; //$incomingTotal = rupiah($dataReceive['incomingTotal']);
							$reductionTotal = $dataReceive['reductionTotal']; //$reductionTotal = rupiah($dataReceive['reductionTotal']);
							$sisa = $dataReceive['receiveTotal'] - ($dataReceive['incomingTotal'] + $dataReceive['reductionTotal']); //$sisa = rupiah($dataReceive['receiveTotal'] - ($dataReceive['incomingTotal'] + $dataReceive['reductionTotal']));
